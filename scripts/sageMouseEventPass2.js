...
var sagemep = {
	...
	*/
	processAndPassEvents : function ( appId, type, position, user, data, date, iFrameElement, debug ) {

		//gets the element under the pointer coordinate.
		var theElementAtThePointer;
		function adjustOffsetForScroll(contentDocument, currentOffset) {
			//handle outermost frame being scrolled
			if (contentDocument.body.scrollLeft != 0 ||contentDocument.body.scrollTop != 0) {
				currentOffset.x -= contentDocument.body.scrollLeft;	
				currentOffset.y -= contentDocument.body.scrollTop;	
				if (debug) console.log('contentDocument was scrolled so adding contentDocument.body.scrollLeft: ' + contentDocument.body.scrollLeft + ', contentDocument.body.scrollTop: ' + contentDocument.body.scrollTop);
				return true;
			}
			return false;
		}
		
		function adjustOffsetForOffset(iFrame, currentOffset) {
			//handle outermost frame being scrolled
			if (iFrame.offsetLeft != 0 || iFrame.offsetTop != 0) {
				currentOffset.x += iFrame.offsetLeft;	
				currentOffset.y += iFrame.offsetTop;	
				if (debug) console.log('iFrame was offset so adding iFrame.offsetLeft: ' + iFrame.offsetLeft + ', iFrame.offsetTop: ' + iFrame.offsetTop)
				return true;
			}
			return false;
		}
		...
		//adjust position to take pointer icon offset into account (don't know where this is defined)
		//This may actually reflect a bug in sage - the event coordinates should be translated so that the point of the viusal cursor is the orignating point. Looks like it is using the center instead.
		offset.x += 8;
		offset.y += 8;

		//handle outermost frame being scrolled
		//adjustOffsetForScroll(iFrameElement.contentDocument, offset);
		
		//adjustOffsetForOffset(iFrameElement, offset);

		// var currentIFrame = iFrameElement;		
		// var currentDocument = iFrameElement.contentDocument
	
		// while (true) {
		// 	theElementAtThePointer = currentDocument.elementFromPoint(position.x - offset.x, position.y - offset.y);
		// 	if (debug) console.log('inside sagemep theElementAtThePointer: ' + theElementAtThePointer + ', id: ' + (theElementAtThePointer ? theElementAtThePointer.id  + ', type: ' +  theElementAtThePointer.nodeType: ''))
			
		// 	if (theElementAtThePointer == null) {
		// 		if (debug) console.log('processAndPassEvents exiting because point.currentElement == null');
		// 		return;
		// 	}
		// 	if (theElementAtThePointer.contentDocument) {
		// 		//if it has a contentDocument must be an iFrame
		// 		currentIFrame = theElementAtThePointer;
		// 		adjustOffsetForOffset(currentIFrame, offset);
				
		// 		//before switching into iframe add the enclosing frames scroll to offset
		// 		adjustOffsetForScroll(currentDocument, offset);
		// 		//now change current document to iFrame
		// 		currentDocument = currentIFrame.contentDocument;
				
		// 		//should do scroll adjustment here but i found that it I did
		// 		//then I got a disconnect between where the click point was and the visible position of the mouse. 
		// 		//adjustOffsetForScroll(currentDocument, offset);
		// 		if (debug) console.log('inside an iframe so changing to its document to search within. Cumulative offset.x: ' + offset.x + ', offset.y: ' + offset.y);
		// 	} else {
		// 		//is the current iframe different than the one we started in?
		// 		if (true || currentIFrame !== iFrameElement) {
		// 		//this scroll adjustment should be in above section when we go into iframe but i found that it I did
		// 		//then I got a disconnect between where the click point was and the visible position of the mouse. 
		// 			adjustOffsetForScroll(currentDocument, offset);
		// 		}
		// 		break;
		// 		}
		// }	//end while loop over iframes
		...
		point.xCurrent = position.x - offset.x;
		point.yCurrent = position.y - offset.y;
		...
		// point.currentElement = theElementAtThePointer;
		...
		switch(type)
		{
			...
			case "pointerMove":
				...
				if(point.currentElement == point.previousElement) {
					mouseEventToPass = new MouseEvent("mousemove", {
	      				...
	      				pageX: point.xCurrent,
	      				pageY: point.yCurrent,
			...
			case "pointerPress":
				if(this.debug) {
					...
					console.log("pointer press at:" + point.xCurrent + "," + point.yCurrent);
			...
			case "pointerDoubleClick":
				...
				if(this.debug) {
					console.log("sagemep: double click"); //does sage send double clicks?
				}
			...
			case "keyboard":
				...
				if(this.debug) {
					console.log("sagemep: Keystroke:" + data.character + "("+data.code+")");
				}
			...
			case "specialKey":
				if(this.debug) {
					console.log("sagemep: specialkey:" + data.character + "("+data.code+")");
				}
				...
				if(elemToSendKeyValuesTo != null && elemToSendKeyValuesTo.value != null) {
					if( elemToSendKeyValuesTo.value.length > 0 && ( data.code === 8 || data.code === 46 ) && data.state === 'up' ) {
						elemToSendKeyValuesTo.value = elemToSendKeyValuesTo.value.substring( 0,  elemToSendKeyValuesTo.value.length -1);
					}
				}
			...
			default:
				if(this.debug) {
					console.log("ERROR from sagemep: Unknown SAGE type:" + type);
				}
				break;
	...
	*/
	determineAndSendEnterEvents : function (point) {
		...
		var cElem = point.currentElement;
		...
		if(isPreviousElementAncestor) {
			...
			while( pElem != null && ! foundCommonAncestor ) {
				failSafeOnInf++;
				if(this.debug && failSafeOnInf > 500 ) {
					console.log("ERROR: sagemep: failsafe break out of finding a common ancestor for event triggers.");
					break;
				}
				...
				while(cElem.nodeName != "HTML") {
					...
					if (cElem.parentNode == null) {
						console.log('cElem.parentNode == null. cElem.nodeName: ' + cElem.nodeName)
						}
					cElem = cElem.parentNode;
				}
			...
			if( !foundCommonAncestor ) {
				if(this.debug) {console.log("ERROR: sagemep: unable to find common ancestor."); }
				return;
			}
			...
			do {
				...
				cElem.dispatchEvent(mouseEventToPass);
				if (cElem.parentNode == null) {
					console.log('2 cElem.parentNode == null. cElem.nodeName: ' + cElem.nodeName)
					}
				cElem = cElem.parentNode;
			} while(pElem != null && cElem != null && cElem != pElem)
	...
	*/
	getElementUnderPointer : function (px, py, appDivId) {
		...
		// if(this.debug) { console.log("deleteme appid:" + appDivId.substring(3) ); }
		...
		*/
		...
		//console.dir(theElementAtThePointer);
