			...
			case "pointerMove":
				...
				if(point.currentElement == point.previousElement) {
					mouseEventToPass = new MouseEvent("mousemove", {
	      				...
	      				clientX: xLocationOfPointerOnScreen,
	      				clientY: yLocationOfPointerOnScreen,
	      				// screenX: point.xCurrent,
	      				screenX: xLocationOfPointerOnScreen,
	      				screenY: yLocationOfPointerOnScreen,
...
// BUG!!! Focus causes browser windows to move
				// point.currentElement.focus(); //focus happens on mousedown
				// point.currentElement.triggerHandler("focus"); //focus happens on mousedown
				// point.currentElement.trigger("focus"); //focus happens on mousedown
			...
			case "pointerRelease":
				if(this.debug) {
					...
					// if(point.currentElement.hasAttribute("qlikid")){
					// 	// console.log("Clicked on a Qlik Object:",point.currentElement.getAttribute("qlikid"));
					// 	objID = point.currentElement.getAttribute("qlikid");
					// }
				}
				...
				mouseEventToPass = new MouseEvent("mouseup", {
      				...
      				// clientX: point.xCurrent,
      				clientX: xLocationOfPointerOnScreen,
      				clientY: yLocationOfPointerOnScreen+40,
      				// screenX: point.xCurrent,
      				screenX: xLocationOfPointerOnScreen,
      				screenY: yLocationOfPointerOnScreen+40,
      				//relatedTarget: point.previousElement, //for: focus, mouse enter leave out over, drag
      				target: point.currentElement,
				});
				point.currentElement.dispatchEvent(mouseEventToPass);
				...
				if(point.elementPressed == point.currentElement){
					...
					console.log("Clicked on ", point.currentElement);
			...
			case "keyboard":
				...
				if (oEvent.initKeyboardEvent) {
				    oEvent.initKeyboardEvent("keydown", true, true, document.defaultView, false, false, false, false, data.code, data.code);
				} else {
				    oEvent.initKeyEvent("keydown", true, true, document.defaultView, false, false, false, false, data.code, 0);
				}

				oEvent.keyCodeVal = data.code;

				if (oEvent.keyCode !== data.code) {
				    console.log("keyCode mismatch " + oEvent.keyCode + "(" + oEvent.which + ")");
				}

				console.log(elemToSendKeyValuesTo);
				console.log(oEvent);
				elemToSendKeyValuesTo.dispatchEvent(oEvent);
